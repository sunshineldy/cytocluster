 ...
 * 
 * be used to show the visual View
 * 
...
public class NodeDistances implements MonitorableTask {
	...
	protected Map nodeIndexToMatrixIndexMap; // a root node index to matrix
												// index map
	...
	public NodeDistances(List nodesList, CyNetwork network,
			Map nodeIndexToMatrixIndexMap) {
	...
	/*
	 * public NodeDistances(List nodes_list, int[][] distances, GraphPerspective
	 * perspective) { this(nodes_list, distances, perspective, false);
	 * 
	 * }//NodeDistances
	 */
	/**
	...
	/*
	 * public NodeDistances(List nodes_list, int[][] distances, GraphPerspective
	 * perspective, boolean directed) { this.perspective = perspective;
	 * nodesList = nodes_list; if (distances == null) { this.distances = new
	 * int[nodesList.size()][]; } else { this.distances = distances; }
	 * this.directed = directed; }//NodeDistances
	 */
	/**
	 ...
	 * HashMap nodeIndexToMatrixIndexMap) { this.nodesList = nodesList;
	 * this.nodeIndexToMatrixIndexMap = nodeIndexToMatrixIndexMap;
	 * this.perspective = perspective; this.distances = new
	 * int[nodesList.size()][]; this.directed = false; }
	 */
	/**
	...
	public int[][] calculate() {
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			int index = ((Integer) nodeIndexToMatrixIndexMap.get(from_node
					.getSUID())).intValue();
			// index = ((Integer) nodeIndexToMatrixIndexMap.get(new
			// Integer(from_node.getRootGraphIndex()))).intValue();

			if ((index < 0) || (index >= nodes.length)) {
				JOptionPane.showMessageDialog(null, "WARNING: GraphNode \""
						+ from_node
			...
			if (nodes[index] != null) {
				JOptionPane.showMessageDialog(null, "WARNING: GraphNode \""
						+ from_node + "\" has an index value ( " + index
						+ " ) that is the same as "
			...
			while (!(queue.isEmpty())) {
				...
				if (index < from_node_index) {
					...
					for (int i = 0; i < nodes.length; i++) {
						...
						// distance from
						// to_node.
						// So now we don't need to put any neighbors on the
						// queue or
						// anything, since they've already been taken care of by
						// the previous
						// calculation.
					continue;
				...
				while (neighbors.hasNext()) {
					...
					// nodeIndexToMatrixIndexMap.get(new
	...
	// get Collectioin from one CyNode
	private Collection getNeighbors(CyNode node) {
		Set result = new HashSet();
		Collection edges = network.getAdjacentEdgeList(node, CyEdge.Type.ANY);
		if (edges == null || edges.size() == 0)
			return result;
		Long nodeID = node.getSUID(); // the node's SUID
		for (Iterator iterator = edges.iterator(); iterator.hasNext();) {
			CyEdge curEdge = (CyEdge) iterator.next();
			if (curEdge.getSource().getSUID() != nodeID) // sourceNode
				result.add(curEdge.getSource());
			else if (curEdge.getTarget().getSUID() != nodeID) // targetNode
