...
/**
 * An implementation of spring embedded layout algorithm. Note 1: this was
 * copied from giny.util because it is being phased out. Eventually the layout
 * API will be available to use (TODO: remove layout when layout API is
 * available) Note 2: this has been modified so that the doLayout method is
 * interruptable and reports progress to the Loader
 * */
public class ClusterLayout {
	...
	public boolean doLayout(int weightLayout, int goalTotal, double progress,
		...
		/*
		 * int count=0; while (nodes.hasNext()) { NodeView nodeView = (NodeView)
		 * nodes.next(); nodeIndexToMatrixIndexMap.put(new
		 * Integer(nodeView.getRootGraphIndex()), new Integer(count));
		 * matrixIndexToNodeIndexMap.put(new Integer(count), new
		 * Integer(nodeView.getRootGraphIndex())); count++; }
		 */
		...
		for (layoutPass = 0; layoutPass < numLayoutPasses; layoutPass++) {
			 ...
			 * calculatePartials(partials, null, potential_energy, false);
			 * partials_list.add(partials); if ((furthest_node_partials == null)
			 * || (partials.euclideanDistance >
			 * furthest_node_partials.euclideanDistance) ) {
			 * furthest_node_partials = partials; } }
			 */

			// Until num_iterations, or the farthest node is not-so-far, move
			// the
			...
			for (int iterations_i = 0; ((iterations_i < num_iterations) && (furthest_node_partials.euclideanDistance >= euclidean_distance_threshold)); iterations_i++) {
				if (interrupted) {
					...
					// Before we short circuit the method, we reset the
				...
				// energy by moving the node that is farthest from where it
				// should be.
		} // End for each layout pass
			// Just in case an interruption occurred right before we exit the
			// method, we reset it, such an interruption
	...
	protected void setupNodeDistanceSprings() {
		...
		for (Iterator iterator = matrixIndices.iterator(); iterator.hasNext();) {
			...
			nodeList.add(i, ((CyNetwork) graphView.getModel())
					.getNode(nodeIndex.longValue()));
			/*
			 * nodeList.add(i,
			 * graphView.getGraphPerspective().getNode(nodeIndex.intValue()));
			 */
			i++;
		}
		...
		/*
		 * NodeDistance ind = new NodeDistance(nodeList,
		 * graphView.getGraphPerspective(), nodeIndexToMatrixIndexMap);
		 */
	...
	protected PartialDerivatives calculatePartials(PartialDerivatives partials,
		...
		while (iterator.hasNext()) {
			...
			if (((CyNode) node_view.getModel()).getSUID() == ((CyNode) other_node_view
				...
				// other_node_view.getRootGraphIndex()) { //System.out.println(
				// "Nodes are the same. " );
				continue;
			}
