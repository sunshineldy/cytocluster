...
/**
 * A loader for loading picture with a progress bar for the complex browser
 * table
 * 
 * the progress bar
 */
public class Loader extends ImageIcon implements Runnable {
	...
	/**
	 * Generates a loading picture with a progress bar for the complex browser
	 * table
	 * 
	 * @param table
	 *            Reference to the cluster browser table
	 * @param width
	 *            cell width
	 * @param height
	 *            cell height
	...
	public Loader(int selectedRow, JTable table, int width, int height) {
		...
		this.offScreenG2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				RenderingHints.VALUE_ANTIALIAS_ON);

		setImage(this.onScreenLoader);
	}

	public Loader(JTable table, int width, int height) {
		this.table = table;
	 ...
	 * 
	 * @param selectedRow
	 *            the row in a table where the loader is to be drawn
	 * @param table
	 *            an image of the cluster before the loader is drawn on top
	 */
	 ...
	 * string and an animated disk to indicate responsiveness as well as a
	 * progress bar and process status.
	 */
	private void drawLoader() {
		// We want to compute this image off screen so that partially completed
		// images are never displayed
		...
		int graphX = Math.round((((float) loader.getWidth() - graphImage
				.getIconWidth()) / 2));
		int graphY = Math.round((((float) loader.getHeight() - graphImage
				.getIconHeight()) / 2));

		g2.drawImage(graphImage.getImage(), graphX, graphY,
				this.getImageObserver());

		// Fade background on top to make it look like the graph is fading away
		// while the loading text fades in
		...
		g2.setColor(bgColor);
		g2.setColor(new Color(g2.getColor().getRed(), g2.getColor().getGreen(),
				g2.getColor().getBlue(), (int) (200.0 * fadeInAlpha)));
		g2.fillRect(0, 0, loader.getWidth(), loader.getHeight());
		...
		Ellipse2D circOutter = new Ellipse2D.Double(
				(loader.getWidth() / 2) - r, (loader.getHeight() / 2) - r,
				2 * r, 2 * r);
		...
		// To produce the fading effect we must draw consecutively more
		// transparent polygons around the disk
		// this vriable keeps track of the fading polygons
		// at the start of each frame we want to first draw the least
		// transparent polygon in the same position on the rotating kisk
		double degreesForTrail = degreesForDisk;
		// Here we find the center of the table cell
		Point2D center = new Point2D.Double(loader.getWidth() / 2,
				loader.getHeight() / 2);
		// these two points will represent the outer ends of the polygon,
		...
		// In order for the leading portion of the rotating disk to be one color
		// and the trailing portion another we use
		...
		while (((int) markerAlpha) > 0) {
			...
			double theta2 = 2 * Math.PI * ((degreesForTrail - 6.0) / 360);// offset
																			// by
																			// 5
																			// degrees
			// the outter spinning point locations can be determined by the
			// circle equations here
			pointOnCircumference1.setLocation(
					((r * Math.cos(theta1))) + center.getX(),
					((r * Math.sin(theta1))) + center.getY());
			pointOnCircumference2.setLocation(
					((r * Math.cos(theta2))) + center.getX(),
					((r * Math.sin(theta2))) + center.getY());
			// this is the color with the decrementing alpha
			g2.setColor(new Color(
					(int) ((markerLeadColor.getRed() * markerLeadColorWeighting) + (markerTrailColor
							.getRed() * (1.0 - markerLeadColorWeighting))),
					(int) ((markerLeadColor.getGreen() * markerLeadColorWeighting) + (markerTrailColor
							.getGreen() * (1.0 - markerLeadColorWeighting))),
					(int) ((markerLeadColor.getBlue() * markerLeadColorWeighting) + (markerTrailColor
							.getBlue() * (1.0 - markerLeadColorWeighting))),
					(int) Math.rint(markerAlpha)));
			if ((int) (markerLeadColorWeighting * 100) > 0) {
				markerLeadColorWeighting = markerLeadColorWeighting
						- (markerLeadColorWeighting / 40);
			}
			// polygons need arrays of x and y values to be drawn, so these are
			// triangles consisting of a center point and two outter points
			int[] xs = { (int) center.getX(),
					(int) pointOnCircumference1.getX(),
					(int) pointOnCircumference2.getX() };
			int[] ys = { (int) center.getY(),
					(int) pointOnCircumference1.getY(),
					(int) pointOnCircumference2.getY() };
			...
			// the successive, more transparent marker is offset by 2 degrees
			// backward to give the appearance of motion blur
			degreesForTrail -= 2;
		}
		// The successive disk is rotated 15 degrees for an optimal speed given
		// the fps
		degreesForDisk += 15;
		...
		g2.setColor(new Color(g2.getColor().getRed(), g2.getColor().getGreen(),
				g2.getColor().getBlue(), (int) (255.0 * fadeInAlpha)));
		g2.drawString(
				loadingText,
				(loader.getWidth() / 2) - (fm.stringWidth(loadingText) / 2) - 1,
				(loader.getHeight() / 2) + (fontSize / 2) - 1);
		g2.drawString(
				loadingText,
				(loader.getWidth() / 2) - (fm.stringWidth(loadingText) / 2) - 1,
				(loader.getHeight() / 2) + (fontSize / 2) + 1);
		...
		g2.setColor(new Color(g2.getColor().getRed(), g2.getColor().getGreen(),
				g2.getColor().getBlue(), (int) (255.0 * fadeInAlpha)));
		g2.drawString(loadingText,
				(loader.getWidth() / 2) - (fm.stringWidth(loadingText) / 2),
				(loader.getHeight() / 2) + (fontSize / 2));
		...
		// The transparency of the bar occilates between 100 and 150 using the
		// sin function and the degrees of the rotating disk
		// completing one period with every turn of the disk
		int progressBarAlpha = 150 - (int) (50.0 * Math.abs(Math.sin(2.0
				* Math.PI * (((double) degreesForDisk / 2) / 360.0))));
		g2.setColor(new Color(g2.getColor().getRed(), g2.getColor().getGreen(),
				g2.getColor().getBlue(), progressBarAlpha));
		g2.fillRect(10, loader.getHeight() - 20,
				(int) ((((double) progress) / 100) * (loader.getWidth() - 20)),
				10);

		// Progress Bar Outline
		g2.setColor(fgColor);
		g2.setColor(new Color(g2.getColor().getRed(), g2.getColor().getGreen(),
				g2.getColor().getBlue(), 100));
		g2.drawRect(10, loader.getHeight() - 20, loader.getWidth() - 20, 10);

		// Progress Text
		String progressText = progress + "%";
		// We add a black shadow to make it easier to read on top of any
		// background
		g2.setColor(bgColor);
		g2.drawString(progressText,
				(loader.getWidth() / 2) - (fm.stringWidth(progressText) / 2)
						+ 1, loader.getHeight() - 11);
		...
		// Lastly we paint the computed image onto the onscreen picture for
		// display
	 ...
	 * @param process
	 *            a short description of the process being conducted (initially
	 *            set to "Starting")
	 */
	...
	public void start() {
		this.graphImage = ((ImageIcon) this.table.getValueAt(this.selectedRow,
				0));
