...
public class CurrentParameters {
	...
	public void setParams(ParameterSet newParams, int resultId, Long networkID) {
		ParameterSet currentParamSet = new ParameterSet(newParams.getScope(),
				newParams.getSelectedNodes(), newParams.isIncludeLoops(),
				newParams.getDegreeCutoff(), newParams.getKCore(),
				newParams.isOptimize(), newParams.getMaxDepthFromStart(),
				newParams.getNodeScoreCutoff(), newParams.isFluff(),
				newParams.isHaircut(),
				...
				newParams.getShortestPathLength(), newParams.getTinThreshold(),
		...
		ParameterSet resultParamSet = new ParameterSet(newParams.getScope(),
				newParams.getSelectedNodes(), newParams.isIncludeLoops(),
				newParams.getDegreeCutoff(), newParams.getKCore(),
				newParams.isOptimize(), newParams.getMaxDepthFromStart(),
	...
	public ParameterSet getResultParams(int resultId) {
		return ((ParameterSet) this.resultParams.get(Integer.valueOf(resultId)))
				.copy();
	}
