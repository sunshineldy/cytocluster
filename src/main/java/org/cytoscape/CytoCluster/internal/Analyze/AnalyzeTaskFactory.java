...
public class AnalyzeTaskFactory implements TaskFactory {
	...
	public AnalyzeTaskFactory(CyNetwork network, int analyze, int resultId,
	...
	public TaskIterator createTaskIterator() {
		return new TaskIterator(new Task[] { new AnalyzeTask(this.network,
				this.analyze, this.resultId, this.alg, this.mcodeUtil,
				this.listener) });
	}

	public boolean isReady() {
		/*
		 * boolean isR = (network==null); isR
		 * &=this.mcodeUtil.getCurrentParameters
		 * ().getParamsCopy(network.getSUID()).getAlgorithm().isEmpty();
		 * 
		 * return isR;
		 */
		return true;
