...
public class EAGLE extends Algorithm {
	...
	public void getMaximalCliques(CyNetwork inputNetwork, int resultTitle) {
		...
		if (!maximalCliquesNetworkMap.containsKey(inputNetwork.getSUID())) {
			...
			for (Iterator i = inputNetwork.getNodeList().iterator(); i
					.hasNext();) {
	...
	public Cluster[] EAGLEFinder(CyNetwork inputNetwork, int resultTitle) {
		...
		currentNetwork = inputNetwork;
		params = getParams();
		curCliques = (HashMap) maximalCliquesNetworkMap.get(inputNetwork
				.getSUID());
		if (curCliques == null) {
			System.err.println("In " + callerID
					+ ": maximal cliques Map was null.");
			return (null);
		}
		long msTimeBefore = System.currentTimeMillis();
		...
		if (optimalDivisionKeyMap.containsKey(key)) {
			...
			for (int i = 0; i < curCliques.size(); i++) {
				...
				if (cur.isSubordinate()) {
					...
					while (it.hasNext()) {
						...
						if (searchInComplexes(alClusters, new Long(n)) == 0) { // a
																				// subordinate
																				// node
							Cluster newCluster = new Cluster();
							ArrayList alNodes = new ArrayList();
			...
			double max = 0, cur;
			int num = 0;
			// merge two complexes with largest similarity each time until there
			// is only on left
			do {
		...
		while (it.hasNext()) {
			...
			if (cluster.getALNodes().size() >= params
				...
				ClusterGraph gpCluster = createClusterGraph(alNodes,
						inputNetwork);
	 ...
	 * 
	 * @param complexes
	 *            the list of complexes to be merged
	 */
	private void mergeComplex(ArrayList complexes)// the number of complexes
													// must be no less than two
	{
		double max = -100000, simval;
		int index1 = 0, index2 = 1;
		int i, j, flag;
		for (i = 0; i < complexes.size() - 1; ++i) {// for each pair of
													// complexes,calculate their
													// similarity
			for (j = i + 1; j < complexes.size(); ++j) {
				simval = calSimilarity((Cluster) complexes.get(i),
						(Cluster) complexes.get(j));
				if (simval > max) {
					max = simval;
