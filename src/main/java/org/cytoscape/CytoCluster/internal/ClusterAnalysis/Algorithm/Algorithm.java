...
public abstract class Algorithm {
	protected boolean cancelled = false;// If set, will schedule the canceled
										// algorithm at the next convenient
										// opportunity
	protected TaskMonitor taskMonitor = null;
	protected ParameterSet params; // the parameters used for this instance of
									// the algorithm
	...
	// data structures useful to storing information for more than one cluster
	// finding iteration
	protected HashMap<Long, NodeInfo> curNodeInfos = null; // vector<Node> key
															// is the node
															// index, value is a
															// NodeInfo instance
	protected TreeMap<Double, List<Long>> curNodeScores = null; // key is node
																// score, value
																// is nodeIndex
	protected TreeMap curEdgeWeights = null; // key is edge index,value is
												// EdgeInfo instance
	protected HashMap curCliques = null; // key is clique ID,value is Clique
											// instance
	protected ArrayList curOptimalDivision = null;

	protected HashMap<Integer, Map<Long, NodeInfo>> nodeInfoResultsMap = new HashMap<Integer, Map<Long, NodeInfo>>(); // key
																														// is
																														// result,
																														// value
																														// is
																														// nodeInfroHashMap
	protected HashMap<Integer, SortedMap<Double, List<Long>>> nodeScoreResultsMap = new HashMap<Integer, SortedMap<Double, List<Long>>>();// key
																																			// is
																																			// result,
																																			// value
																																			// is
	...
	protected HashMap maximalCliquesNetworkMap = new HashMap(); // key is
																// networkID,
																// value is
																// maximal
																// Cliques
	 ...
	 * 
	 * @param networkID
	 *            the algorithm use it to get the parameters of the focused
	 *            network
	 */
	public Algorithm(Long networkID, ClusterUtil clusterUtil) {

		this.clusterUtil = clusterUtil;
		this.params = this.clusterUtil.getCurrentParameters().getParamsCopy(
				networkID);
	}
	 ...
	 * 
	 * @param cur
	 *            stands for the currently growing clique
	 * @param cand
	 *            the candidate nodes to be added, they are common neighbors of
	 *            nodes in cur
	 * @param not
	 *            nodes already processed, so there is no common elements
	 *            between cand and not
	 */
	protected void expand(HashMap cliques, Vector curr, Vector cand, Vector not) {
		if (cand.isEmpty() && not.isEmpty()) {// the expanding process has come
												// to an end
			int num = cliques.size();
			Clique pc = new Clique(num); // the node in cur can form a new
											// maximal clique
			...
			cliques.put(new Integer(pc.getCliuqueID()), pc); // add to the
																// maximal
																// clique list
		} else {
			Long p;
			...
			for (Iterator it = cuts.iterator(); it.hasNext();)
				// get the trimmed candidates
			...
			for (i = 0; i < cutcand.size(); ++i)// for each non-adjacent node
			{
				...
				ArrayList adjs = getNeighbors(q.longValue());// 1.2 get the
																// adjacent
				...
				curr.remove(curr.lastElement());// pop the top element a new
												// cursive process
				if (cancelled) {
					break;
				}
			...
			/*
			 * if (taskMonitor != null) { i++; taskMonitor.setProgress((i * 100)
	...
	/**
	 * Choose a vertex from cand&not with the largest number of connections to
	 * the vertices in cand
	 */
	private Long getPivot(Vector cand, Vector not) {
		...
		ret = new Long(-1);
		ArrayList neighbors;
		// if(cand.size()==1) //if there is only one node in cand, then we
		// simply choose it
	...
	protected ArrayList getNeighbors(Long nodeIndex) {
		...
		for (CyNode node : currentNetwork.getNeighborList(
				currentNetwork.getNode(nodeIndex), CyEdge.Type.ANY)) {
	 ...
	 * 
	 */
	public static Long[] getNeighborArray(CyNetwork network, Long nodeIndex) {

		ArrayList ret = new ArrayList();
		for (CyNode node : network.getNeighborList(network.getNode(nodeIndex),
				CyEdge.Type.ANY)) {
			ret.add(node.getSUID());
		}
	 ...
	 * @param b
	 *            a arraylist
	 * @return a Vector ----- the intersect set of (a กษ b)
	 */
	 ...
	 * @param a
	 * @param b
	 * @return true:a is different form b
	 */
	...
	/**
	 * calculate the similarities between a pairs of complexes S=1/2m*(
	 * sumof(Aij-ki*kj/2m) ) of which i IEO C1,j IEO C2, and i!=j
	 */
	protected double calSimilarity(Cluster c1, Cluster c2) {
		...
		for (Iterator it1 = nodes1.iterator(); it1.hasNext();) {
			...
			for (Iterator it2 = nodes2.iterator(); it2.hasNext();) {
				...
				if (n1.longValue() != n2.longValue()) {// n1 and n2 is not the
														// same node

					degree2 = getNodeDegree(currentNetwork, n2);

					// degree2=currentNetwork.getDegree(n2.intValue());
					A = (Arrays.binarySearch(neigh, n2.longValue()) < 0) ? 0
							: 1;
					S += A;
	...
	/**
	 * evaluate the quality of the resulting complexes,by which we decide which
	 * division is optimal EQ=1/2m*(sumof( (Aij-ki*kj/2m)/(Oi*Oj) )) of which
	 * i,j IEO Cx
	 * 
	 * @param alClusters
	 *            all the clusters at present
	 */
	...
	/**
	 * K-Core Algorithm Step 1: Score the graph and save scores as node
	 * attributes. Scores are also saved internally in your instance of
	 * Algorithm.
	 * 
	 * @param inputNetwork
	...
	public void scoreGraph(CyNetwork inputNetwork, int resultTitle) {
		...
		HashMap<Long, NodeInfo> nodeInfoHashMap = new HashMap<Long, NodeInfo>(
				inputNetwork.getNodeCount());
		...
		while (nodes.hasNext() && (!cancelled)) {
			...
			// nodeInfoHashMap.put(new Integer(n.getRootGraphIndex()),
			// nodeInfo);
			...
			if (taskMonitor != null) {
				i++;
				taskMonitor
						.setProgress((i * 100) / inputNetwork.getNodeCount());
			}
		}
	...
	/**
	 * K-Core Algorithm Step 2: Find all complexes given a scored graph. If the
	 * input network has not been scored, this method will return null. This
	 * method is called when the user selects network scope or single node
	 * scope.
	 * 
	 * @param inputNetwork
	...
	public Cluster[] K_CoreFinder(CyNetwork inputNetwork, int resultTitle) {
		...
		if (!nodeScoreResultsMap.containsKey(resultTitle)) {// use the node
															// score used last
															// time
		...
		for (Iterator iterator = values.iterator(); iterator.hasNext();) {
			...
			for (int j = 0; j < alNodesWithSameScore.size(); j++) {
				...
				if (!nodeSeenHashMap.containsKey(currentNode)) {
					...
					Map nodeSeenHashMapSnapShot = new HashMap(nodeSeenHashMap);

					ArrayList alNodes = getClusterCore(currentNode,
					...
					if (alNodes.size() > 0) {
						...
						ClusterGraph clusterGraph = createClusterGraph(alNodes,
								inputNetwork);
						...
						if (!filterCluster(clusterGraph)) {
							...
							if (this.params.isFluff()) {
								fluffClusterBoundary(alNodes, nodeSeenHashMap,
										nodeInfoHashMap);
							}
							...
							clusterGraph = createClusterGraph(alNodes,
									inputNetwork);

							currentCluster.setGraph(clusterGraph);
							currentCluster
									.setClusterScore(scoreCluster(clusterGraph));
							// //store all the nodes that have already been seen
							// and incorporated in other clusters
							currentCluster
									.setNodeSeenHashMap(nodeSeenHashMapSnapShot);
	...
	protected void calNodeInfos(CyNetwork net) {
		...
		while (nodes.hasNext() && (!cancelled)) {
			...
			nodeInfo.nodeNeighbors = ClusterUtil
					.convertIntArrayList2array(adjs);
	...
	/**
	 * Calculates node information for each node. This information is used at
	 * the first stage of the K-Clique algorithm. This is a utility function for
	 * the algorithm.
	 * 
	 * @param inputNetwork
	...
	private NodeInfo calcNodeInfo(CyNetwork inputNetwork, Long nodeIndex) {
		...
		List neighborsL = inputNetwork.getNeighborList(rootNode,
				CyEdge.Type.ANY);
		...
		if (Arrays.binarySearch(neighbors, nodeIndex) < 0) {// add itself as its
															// neighbor
		...
		/*
		 * Set nodes = new HashSet(); for(int i=0;i<neighbors.length;i++)
		 * nodes.add((Long)neighbors[i]);
		 */

		ClusterGraph gpNodeNeighborhood = clusterUtil.createGraph(inputNetwork,
				neighborsL);

		// GraphPerspective gpNodeNeighborhood =
		// inputNetwork.createGraphPerspective(neighborhood);
		...
		if (kCore != null) {
			nodeInfo.coreDensity = calcDensity(kCore,
					this.params.isIncludeLoops());
		}

		/*
		 * GraphPerspective gpCore = null; Integer k = null; Object[]
		 * returnArray = getHighestKCore(gpNodeNeighborhood); k = (Integer)
		 * returnArray[0]; gpCore = (GraphPerspective) returnArray[1];
		 * nodeInfo.coreLevel = k.intValue(); //calculate the core density -
		 * amplifies the density of heavily interconnected regions and
		 * attenuates //that of less connected regions if (gpCore != null) {
	...
	/**
	 * Score node using the formula from original paper. This formula selects
	 * for larger, denser cores. This is a utility function for the algorithm.
	 * 
	 * @param nodeInfo
	...
	/*
	 * private double scoreNode(NodeInfo nodeInfo) { if
	 * (nodeInfo.numNodeNeighbors > params.getDegreeCutoff()) { nodeInfo.score =
	 * nodeInfo.coreDensity * (double) nodeInfo.coreLevel; } else {
	 * nodeInfo.score = 0.0; } return (nodeInfo.score); }
	 */
	/**
	 * Gets the calculated node score of a node from a given result. Used in
	 * ResultsPanel during the attribute setting method.
	 * 
	...
	public double getNodeScore(Long nodeId, int resultId) {
		Map nodeScoreSortedMap = (Map) this.nodeScoreResultsMap.get(Integer
				.valueOf(resultId));

		for (Iterator localIterator = nodeScoreSortedMap.keySet().iterator(); localIterator
				.hasNext();) {
			double nodeScore = ((Double) localIterator.next()).doubleValue();
			List nodes = (List) nodeScoreSortedMap.get(Double
					.valueOf(nodeScore));
	 ...
	 * Double nodeScore = new Double(0.0); TreeMap nodeScoreSortedMap =
	 * (TreeMap) nodeScoreResultsMap.get(resultTitle); for (Iterator score =
	 * nodeScoreSortedMap.keySet().iterator(); score.hasNext();) { nodeScore =
	 * (Double) score.next(); ArrayList nodes = (ArrayList)
	 * nodeScoreSortedMap.get(nodeScore); if (nodes.contains(new
	 * Integer(rootGraphIndex))) { return nodeScore; } } return nodeScore; }
	 */
	/**
	 * Gets the highest node score in a given result. Used in the
	 * VisualStyleAction class to re-initialize the visual calculators.
	 */
	 ...
	 */
	public ClusterGraph createClusterGraph(ArrayList<Long> alNode,
			CyNetwork inputNetwork) {
		// convert Integer array to int array
		/*
		 * int[] clusterArray = new int[alNode.size()]; for (int i = 0; i <
		 * alNode.size(); i++) { int nodeIndex = ((Integer)
		 * alNode.get(i)).intValue(); clusterArray[i] = nodeIndex; }
		 */
		...
		ClusterGraph clusterGraph = clusterUtil
				.createGraph(inputNetwork, nodes);

		// ClusterGraph gpCluster =
		// inputNetwork.createGraphPerspective(clusterArray);
		return clusterGraph;
	}
	 ...
	 */
	private ArrayList getClusterCore(Long seedNode, HashMap nodeSeenHashMap,
		...
		getClusterCoreInternal(seedNode,
				((NodeInfo) nodeInfoHashMap.get(seedNode)).score,
				nodeSeenHashMap, 1, cluster, nodeScoreCutoff,
	...
	private boolean getClusterCoreInternal(Long startNode,
			double startNodeScore, HashMap nodeSeenHashMap, int currentDepth,
			ArrayList cluster, double nodeScoreCutoff, int maxDepthFromStart,
			HashMap nodeInfoHashMap) {
		...
		for (i = 0; i < (((NodeInfo) nodeInfoHashMap.get(startNode)).numNodeNeighbors); i++) {
			...
			currentNeighbor = new Long(
					((NodeInfo) nodeInfoHashMap.get(startNode)).nodeNeighbors[i]);
			if ((!nodeSeenHashMap.containsKey(currentNeighbor))
				...
				getClusterCoreInternal(currentNeighbor, startNodeScore,
						nodeSeenHashMap, currentDepth + 1, cluster,
	 ...
	 * @return true
	 */
	private boolean fluffClusterBoundary(ArrayList cluster,
			HashMap nodeSeenHashMap, HashMap nodeInfoHashMap) {
		...
		for (int i = 0; i < cluster.size(); i++) {
			...
			for (int j = 0; j < ((NodeInfo) nodeInfoHashMap.get(new Long(
					currentNode))).numNodeNeighbors; j++) {
				nodeNeighbor = ((NodeInfo) nodeInfoHashMap.get(new Long(
						currentNode))).nodeNeighbors[j];
				if ((!nodeSeenHashMap.containsKey(new Long(nodeNeighbor)))
						&& (!nodeSeenHashMapInternal.containsKey(new Long(
								nodeNeighbor)))
						&& ((((NodeInfo) nodeInfoHashMap.get(new Long(
								nodeNeighbor))).density) > params
								.getFluffNodeDensityCutoff())) {
					nodesToAdd.add(new Long(nodeNeighbor));
					nodeSeenHashMapInternal.put(new Long(nodeNeighbor),
							new Boolean(true));
				}
			}
		}
	 ...
	 * @return true
	 */
	private boolean haircutCluster(ClusterGraph gpClusterGraph,
			ArrayList cluster) {
		// get 2-core
		...
		if (gpCore != null) {
			...
			/*
			 * Long[] rootGraphIndices = gpCore.getNodeIndicesArray(); for (int
			 * i = 0; i < rootGraphIndices.length; i++) { cluster.add(new
			 * Integer
			 * (gpInputGraph.getRootGraphNodeIndex(rootGraphIndices[i]))); }
			 */
		}
		return (true);
	}
	...
	/**
	 * Calculate the density of a graph The density is defined as the number of
	 * edges/the number of possible edges
	 * 
	 * @param gpInputGraph
	...
	public double calcDensity(ClusterGraph gpInputGraph, boolean includeLoops) {
		...
		int actualEdgeNum = getMergedEdgeCount(gpInputGraph, includeLoops);
		...
		// double density = possibleEdgeNum != 0 ? actualEdgeNum /
		// possibleEdgeNum : 0.0D;
		double n = new Double(actualEdgeNum) / (new Double(possibleEdgeNum));
		...
		/*
		 * String callerID = "Algorithm.calcDensity"; if (gpInputGraph == null)
		 * { System.err.println("In " + callerID + ": gpInputGraph was null.");
		 * return (-1.0); } if (includeLoops) { //count loops Iterator nodes =
		 * gpInputGraph.getNodeList().iterator(); while (nodes.hasNext()) {
		 * CyNode n = (CyNode) nodes.next(); if (gpInputGraph.isNeighbor(n, n))
		 * { loopCount++; } } possibleEdgeNum = gpInputGraph.getNodeCount() *
		 * gpInputGraph.getNodeCount(); actualEdgeNum =
		 * gpInputGraph.getEdgeCount() - loopCount; } else { possibleEdgeNum =
		 * gpInputGraph.getNodeCount() * gpInputGraph.getNodeCount();
		 * actualEdgeNum = gpInputGraph.getEdgeCount(); }
		 * 
		 * density = (double) actualEdgeNum / (double) possibleEdgeNum; return
		 * (density);
		 */
	}
	...
	public ClusterGraph getKCore(ClusterGraph gpInputGraph, int k) {
		...
		while (true) {
			numDeleted = 0;
			ArrayList alCoreNodeIndices = new ArrayList(
			...
			while (nodes.hasNext()) {
				...
				degree = gpInputGraph.getAdjacentEdgeList(n, CyEdge.Type.ANY)
						.size();

				if (degree >= k) {
					alCoreNodeIndices.add(new Long(n.getSUID())); // contains
																	// all nodes
																	// with
																	// degree >=
																	// k
			...
			if ((numDeleted > 0) || (firstLoop)) {
				...
				gpOutputGraph = clusterUtil.createGraph(
						gpInputGraph.getRootNetwork(), outputNodes);
				// gpOutputGraph =
				// gpInputGraph.createGraphPerspective(outputNodeIndices);
	...
	/**
	 * Finds the cluster based on user's input via size slider. this function is
	 * only called in ResultPanel.SizeAction
	 * 
	 * @param cluster
	 ...
	 */
	public Cluster exploreCluster(Cluster cluster, double nodeScoreCutoff,
			CyNetwork inputNetwork, int resultId) {
		System.out.println(nodeScoreCutoff + "nodeScoreCutoff");

		HashMap nodeInfoHashMap = (HashMap) nodeInfoResultsMap.get(Integer
				.valueOf(resultId));
		this.params = this.clusterUtil.getCurrentParameters().getResultParams(
				cluster.getResultTitle());
		// ParameterSet params =
		// ParameterSet.getInstance().getResultParams(cluster.getResultTitle()).copy();
		...
		ArrayList alNodes = getClusterCore(seedNode, nodeSeenHashMap,
				nodeScoreCutoff, params.getMaxDepthFromStart(), nodeInfoHashMap);
		if (!alNodes.contains(seedNode))// make sure seed node is part of
										// cluster, if not already in there
		...
		// ClusterGraph gpCluster = Algorithm.createGraphPerspective(alNodes,
		// inputNetwork);
		if (params.isHaircut())
		...
		Cluster newCluster = new Cluster(resultId, seedNode, gpCluster, score,
				alNodes, nodeSeenHashMap);
	...
	static public float getNodeDegreeWeight(CyNetwork network, Long node) {
		...
		Iterator edges = network.getAdjacentEdgeList(cynode, CyEdge.Type.ANY)
				.iterator();
		while (edges.hasNext()) {

			CyEdge e = (CyEdge) edges.next();
			if (network.getRow(e).get("weight", Double.class) != null) {
				// System.out.println(network.getRow(e).get("weight",
				// Double.class));
