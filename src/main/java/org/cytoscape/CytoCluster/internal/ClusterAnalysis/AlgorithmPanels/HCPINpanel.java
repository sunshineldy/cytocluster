...
public class HCPINpanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7268054611643295349L;
	...
	public HCPINpanel(CySwingApplication swingApplication, ClusterUtil mcodeUtil) {
		...
		fThreshold.addPropertyChangeListener("value",
				new HCPINpanel.FormattedTextFieldAction());
		...
		fThreshold.setText((new Double(currentParameters.getfThresholdHCPIN())
				.toString()));
	...
	private JPanel createComplexSizePanel() {
		...
		complexSizeThreshold.setText((new Integer(currentParameters
				.getComplexSizeThresholdHCPIN()).toString()));
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (source == fThreshold) {
				...
				if ((value != null) && (Double.parseDouble(value) >= 1.0)
					...
					currentParameters.setfThresholdHCPIN(Double
							.parseDouble(value));
				} else {
					// source.setValue(new Double
					// (currentParameters.getfThresholdHCPIN()));
				...
				if ((value != null) && (value.intValue() >= 0)) {
					currentParameters.setComplexSizeThresholdHCPIN(value
							.intValue());
				} else {
					// source.setValue(new Double
					// (currentParameters.getComplexSizeThresholdHCPIN()));
