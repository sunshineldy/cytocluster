...
public class FAGECPanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2601246475301117468L;
	...
	public FAGECPanel(CySwingApplication swingApplication, ClusterUtil mcodeUtil) {
		...
		fThreshold.addPropertyChangeListener("value",
				new FAGECPanel.FormattedTextFieldAction());
		...
		fThreshold.setText((new Double(currentParameters.getfThresholdFAGEC())
				.toString()));
		...
		collapsiblePanel
				.setToolTipText("Customize parameters for FAG-EC (Optional)");
		this.add(collapsiblePanel);
	}

	/*
	 * private class FunctionAction extends AbstractAction { public void
	 * actionPerformed(ActionEvent e) { if (weak.isSelected()) {
	 * currentParameters.setWeakFAGEC(true); weakPanel.setVisible(true); } else
	 * { currentParameters.setWeakFAGEC(false); weakPanel.setVisible(false); } }
	 */
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (source == cliqueSizeThreshold) {
				...
				if ((value != null) && (value.intValue() >= 0)
						&& (value.intValue() <= 10)) {
					currentParameters.setCliqueSizeThresholdFAGEC(value
							.intValue());
				} else {
					// source.setValue(new Double
					// (currentParameters.getCliqueSizeThresholdFAGEC()));
				...
				if ((value != null) && (Double.parseDouble(value) >= 1.0)
					...
					currentParameters.setfThresholdFAGEC(Double
							.parseDouble(value));
				} else {
					// source.setValue(new Double
					// (currentParameters.getfThresholdFAGEC()));
				...
				if ((value != null) && (value.intValue() >= 0)) {
					currentParameters.setComplexSizeThresholdFAGEC(value
							.intValue());
				} else {
					// source.setValue(new Double
					// (currentParameters.getComplexSizeThresholdFAGEC()));
	...
	private JPanel createCliqueSizePanel() {
		JPanel panel;
		// the label
		JLabel sizeThresholdLabel = new JLabel(" CliqueSize Threshold"); // Clique
																			// Size
																			// Threshold
																			// input
		...
		cliqueSizeThreshold.addPropertyChangeListener("value",
				new FAGECPanel.FormattedTextFieldAction());
		...
		cliqueSizeThreshold.setText((new Integer(currentParameters
				.getCliqueSizeThresholdFAGEC()).toString()));
		// the panel
		panel = new JPanel() {
			public JToolTip createToolTip() {
				return new MyTipTool();
	...
	private JPanel createComplexSizePanel() {
		...
		panel.setLayout(new BorderLayout());
		// the label
		JLabel sizeThresholdLabel2 = new JLabel(" ComplexSize Threshold"); // Clique
																			// Size
																			// Threshold
																			// input
		// the input text field
		...
		complexSizeThreshold.addPropertyChangeListener("value",
				new FAGECPanel.FormattedTextFieldAction());
		...
		complexSizeThreshold.setText((new Integer(currentParameters
				.getComplexSizeThresholdFAGEC()).toString()));
