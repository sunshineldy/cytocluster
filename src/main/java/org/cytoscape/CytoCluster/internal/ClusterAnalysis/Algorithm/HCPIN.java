...
public class HCPIN extends Algorithm {
	...
	/**
	 * HC-PIN Algorithm Step 1: Calculate arc weights which is defined as (
	...
	public void calEdgeWeight(CyNetwork inputNetwork) {
		...
		if (!edgeWeightNetworkMap.containsKey(networkID)) {
			...
			int aa = 1;
			while (edges.hasNext() && !cancelled) { // for each edge, cal the
													// weight(cluster value)
				...
				double Weight = 0.0f;
				// System.out.println("#######"+aa+"     "+inputNetwork.getRow(From).get("name",
				// String.class)+"##########");
				...
				while (i1.hasNext()) {
					...
					ArrayList aFrom = (ArrayList) inputNetwork
							.getConnectingEdgeList(From, tempe, Type.ANY);
					if (!aFrom.isEmpty()) {
						for (Iterator i = aFrom.iterator(); i.hasNext();) {
							CyEdge a = (CyEdge) i.next();
							if (inputNetwork.getRow(a).get("weight",
									Double.class) != null)
								wsum1 += inputNetwork.getRow(a).get("weight",
										Double.class);
							else
								wsum1 += 1.0;
						}
					}
					ArrayList aTo = (ArrayList) inputNetwork
							.getConnectingEdgeList(To, tempe, Type.ANY);
					if (!aTo.isEmpty()) {
						for (Iterator i = aTo.iterator(); i.hasNext();) {
							CyEdge b = (CyEdge) i.next();
							if (inputNetwork.getRow(b).get("weight",
									Double.class) != null)
								wsum2 += inputNetwork.getRow(b).get("weight",
										Double.class);
							else
								wsum2 += 1.0;
						}
				...
				weight = (wsum1 * wsum2) / (FromWeight * ToWeight); // edge
																	// cluster
																	// value;
				/*
				 * if(weight == 0.6 || weight== 0.5){
				 * System.out.println(weight+" wsum1 = "+wsum1
				 * +" wsum2 = "+wsum2
				 * +" FromWeight = "+FromWeight+" ToWeight= "+ToWeight); }
				 */
				// System.out.println(aa+"     "+inputNetwork.getRow(From).get("name",
				// String.class)+"    "+inputNetwork.getRow(To).get("name",
				// String.class)+"     "+weight);
			 ...
			 * Iterator ii = yy.iterator(); while(ii.hasNext()){
			 * Map.Entry<Double, ArrayList> temp = (Map.Entry<Double,
			 * ArrayList>)ii.next(); Iterator i2 = temp.getValue().iterator();
			 * while(i2.hasNext()){ CyEdge e =
			 * inputNetwork.getEdge((Long)i2.next());
			 * System.out.println(temp.getKey
			 * ()+" ^^^^^^^"+inputNetwork.getRow(e.getSource()).get("name",
			 * String.class)+"  "+inputNetwork.getRow(e.getTarget()).get("name",
			 * String.class)); }
			 * 
			 * }
			 */
	...
	public Cluster[] HCPINFinder(CyNetwork inputNetwork, int resultTitle) {
		...
		if (curEdgeWeights == null || curNodeInfos == null) {
			System.err.println("In " + callerID
					+ ": nodeInfos Map or edgeWeights Map was null.");
			return (null);
		}
		long msTimeBefore = System.currentTimeMillis();
		int findingProgress = 0;
		int findingTotal = 0;
		Collection values = curEdgeWeights.values(); // returns a Collection
														// sorted by key order
														// (descending)
		...
		// input Network
		ArrayList alOriginalClusters = new ArrayList(
		...
		while (nodes.hasNext()) {
			...
			// newCluster.setName(inputNetwork.getRow(n).get("name",
			// String.class));
		...
		for (Iterator iterator = values.iterator(); iterator.hasNext();) {
			// each weight may be associated with multiple edges, iterate over
			// these lists

			/*
			 * class c implements Comparator{ public int compare(Object o1,
			 * Object o2) {
			 * 
			 * CyNode n1t = currentNetwork.getEdge((Long)o1).getTarget(); CyNode
			 * n1s = currentNetwork.getEdge((Long)o1).getSource(); CyNode n2t =
			 * currentNetwork.getEdge((Long)o2).getTarget(); CyNode n2s =
			 * currentNetwork.getEdge((Long)o2).getSource();
			 * 
			 * String s1t = currentNetwork.getRow(n1t).get("name",
			 * String.class); String s1s =
			 * currentNetwork.getRow(n1s).get("name", String.class); String s2t
			 * = currentNetwork.getRow(n2t).get("name", String.class); String
			 * s2s = currentNetwork.getRow(n2s).get("name", String.class);
			 * 
			 * String t1,tt1,t2,tt2; if(s1t.compareTo(s1s)<0){ t1 = s1t; tt1 =
			 * s1s; }else{ tt1 = s1t; t1 = s1s; } if(s2t.compareTo(s2s)<0){ t2 =
			 * s2t; tt2 = s2s; }else{ tt2 = s2t; t2 = s2s; }
			 * if(t1.compareTo(t2)<0) return -1; if(t1.compareTo(t2) == 0)
			 * if(tt1.compareTo(tt2)<0) return -1; return 1; } }
			 */
			alEdgeWithSameWeight = (ArrayList) iterator.next();
			// Collections.sort(alEdgeWithSameWeight,new c());

			for (int j = 0; j < alEdgeWithSameWeight.size(); j++) {// for each
																	// edge
				Long edgeIndex = ((Long) alEdgeWithSameWeight.get(j))
						.longValue();
				curEdge = inputNetwork.getEdge(edgeIndex);
				System.out.println(tt
						+ " "
						+ inputNetwork.getRow(curEdge.getTarget()).get("name",
								String.class)
						+ " "
						+ inputNetwork.getRow(curEdge.getSource()).get("name",
								String.class));
				tt++;
				Long inFrom = curEdge.getSource().getSUID();
				Long inTo = curEdge.getTarget().getSUID();
				NodeInfo fromNI = (NodeInfo) curNodeInfos.get(inFrom); // source
																		// node
																		// info
				NodeInfo toNI = (NodeInfo) curNodeInfos.get(inTo); // target
																	// node info

				int icFrom = fromNI.iComplex; // complex that the source node
												// belongs to
				int icTo = toNI.iComplex; // complex that the target node
											// belongs to
				if (icFrom != icTo) // we have take some actions only if the two
									// complexes are not the same
				{
					Cluster cFrom = (Cluster) alOriginalClusters.get(icFrom);
					Cluster cTo = (Cluster) alOriginalClusters.get(icTo);
					if (cFrom.isMergeable() && cTo.isMergeable()) // the two
																	// complexes
																	// are both
																	// mergeable
						if (!cFrom.isModule() || !cTo.isModule()) // either of
																	// the two
																	// complexes
																	// are not
																	// modules
																	// yet
							if (cFrom.getALNodes().size() >= cTo.getALNodes()
									.size()) {// merge the smaller complexe to
												// the larger one
								if (params.isWeakHCPIN())
							...
							} else { // merge the smaller complex to the larger
										// one
								if (params.isWeakHCPIN())
		...
		while (it.hasNext()) {
			...
			if (cluster.getALNodes().size() >= params
					.getComplexSizeThresholdHCPIN()) {
				ArrayList<Long> alNodes = cluster.getALNodes();
				ClusterGraph gpCluster = this.createClusterGraph(alNodes,
						inputNetwork);
		...
		// case of selection scope,
		// we select only the ones that contain the selected node(s) and return
		// those
		...
		for (int t = 0; t < clusters.length; t++) {
			...
			/*
			 * int x=1; while(ii.hasNext()){ CyNode nn =
			 * inputNetwork.getNode((Long)ii.next());
			 * System.out.println(x+"    "
			 * +inputNetwork.getRow(nn).get("name",String.class)); x++; }
			 */
		}
	...
	protected void mergeComplexes1(Cluster c1, Cluster c2) {
		...
		// System.out.println("  $$$   "+currentNetwork.getRow(u).get("name",
		// String.class));
		// }
	 ...
	 */
	/*
	 * protected void mergeComplexes3(Cluster c1, Cluster c2){ ArrayList
	 * nodes1=c1.getALNodes(); ArrayList nodes2=c2.getALNodes(); //add the
	 * unoverlapped nodes and set the subComplexes of all nodes in C2 NodeInfo
	 * nodeNI; ArrayList subComplexes; for(Iterator
	 * it=nodes2.iterator();it.hasNext();){//for each node in C2 Long
	 * node=(Long)it.next(); nodeNI=(NodeInfo)curNodeInfos.get(node);
	 * subComplexes=nodeNI.getAlComplex(); if(!nodes1.contains(node)){//this is
	 * not a overlapped node int index=subComplexes.indexOf(new
	 * Integer(c2.getComplexID())); subComplexes.remove(index);
	 * subComplexes.add(new Integer(c1.getComplexID())); nodes1.add(node); }
	 * else{ //this node already exists in C1 int index=subComplexes.indexOf(new
	 * Integer(c2.getComplexID())); subComplexes.remove(index); } } //calculate
	 * the other informations for C1 int inDegree=0; int totalDegree=0;
	 * for(Iterator it=nodes1.iterator();it.hasNext();){//for each node in
	 * merged C1 Long node=((Long)it.next()).longValue();
	 * totalDegree+=super.getNodeDegree(currentNetwork,node);
	 * //totalDegree+=currentNetwork.getDegree(node);//can this be useful?
	 * Long[] neighbors=getNeighborArray(currentNetwork,node); for(int
	 * i=0;i<neighbors.length;i++) if(nodes1.contains(neighbors[i])) inDegree++;
	 * } int outDegree=totalDegree-inDegree; inDegree=inDegree/2;
	 * c1.setInDegree(inDegree); c1.setTotalDegree(totalDegree); double fModule
	 * = (double)inDegree/(double)outDegree; if(fModule>params.getfThreshold())
	 * c1.setModule(true); //clear the content of nodes2 nodes2.clear(); } /**
	 * merge overlapped complexes using strong module definition
	 */
	/*
	 * protected void mergeComplexes4(Cluster c1, Cluster c2){ ArrayList
	 * nodes1=c1.getALNodes(); ArrayList nodes2=c2.getALNodes(); //add the
	 * unoverlapped nodes and set the subComplexes of all nodes in C2 NodeInfo
	 * nodeNI; ArrayList subComplexes; for(Iterator
	 * it=nodes2.iterator();it.hasNext();){//for each node in C2 Long
	 ...
	 * else{ //this node already exists in C1 int index=subComplexes.indexOf(new
	 * Integer(c2.getComplexID())); subComplexes.remove(index); } }
	 * c1.setModule(true); int nodeInDegree,nodeTotalDegree; for(Iterator
	 * i=nodes1.iterator();i.hasNext();){ Long
	 * nodeIndex=((Long)i.next()).longValue(); Long[]
	 * adjs=getNeighborArray(currentNetwork,nodeIndex); nodeInDegree=0; for(int
	 * j=0;j<adjs.length;j++) if(nodes1.contains(adjs[j])) nodeInDegree++;
	 * 
	 * 
	 * nodeTotalDegree=super.getNodeDegree(currentNetwork,nodeIndex); //
	 * nodeTotalDegree=currentNetwork.getDegree(nodeIndex); float fModule =
	 * (float)nodeInDegree/(float)(nodeTotalDegree); if(fModule<0.5){
	 * c1.setModule(false); } } //clear the content of nodes2 nodes2.clear(); }
	 * 
	 * 
	 * /** EAGLE Step1 and FEA-EC Step2(optional) get Maximal Cliques: get all
	 * the maximal cliques in the network
	 * 
	 * @param inputNetwork the operated network
	 */
	/*
	 * public void getMaximalCliques(CyNetwork inputNetwork,int resultTitle){
	 * String callerID = "Algorithm.getMaximalCliques"; long
	 * startTime=System.currentTimeMillis(); if (inputNetwork == null) {
	 * System.err.println("In " + callerID + ": inputNetwork was null.");
	 * return; } currentNetwork=inputNetwork; params=getParams();
	 * 
	 * Long net=inputNetwork.getSUID();
	 * if(!maximalCliquesNetworkMap.containsKey(inputNetwork.getSUID())){
	 * System.out.println("Get MaximalCliques for This Network........"); long
	 * msTimeBefore = System.currentTimeMillis(); HashMap cliques = new
	 * HashMap();
	 * 
	 * //initialize states Vector alCur=new Vector(); Vector alFini=new
	 * Vector(); Vector alNodes=new Vector(inputNetwork.getNodeCount());
	 * for(Iterator i=inputNetwork.getNodeList().iterator();i.hasNext();){ Long
	 * node=new Long(((CyNode)i.next()).getSUID()); alNodes.add(node); } //The
	 * critical internal process expand(cliques,alCur,alNodes,alFini);
	 * 
	 * curCliques=cliques; maximalCliquesNetworkMap.put(net, cliques);
	 * findCliquesTime=System.currentTimeMillis()-msTimeBefore; } else
