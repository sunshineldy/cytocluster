...
public class EAGLEpanel extends JPanel {
	...
	public EAGLEpanel(CySwingApplication swingApplication, ClusterUtil mcodeUtil) {
		...
		collapsiblePanel.getContentPane().add(cliqueSizePanel,
				BorderLayout.NORTH);
		collapsiblePanel.getContentPane().add(complexSizePanel,
				BorderLayout.CENTER);
		collapsiblePanel
				.setToolTipText("Customize parameters for EAGLE (Optional)");
	...
	private JPanel createCliqueSizePanel() {
		...
		cliqueSizeThreshold.addPropertyChangeListener("value",
				new EAGLEpanel.FormattedTextFieldAction());
		...
		cliqueSizeThreshold.setText((new Integer(currentParameters
				.getCliqueSizeThresholdEAGLE()).toString()));
		// the panel
		panel = new JPanel() {
			public JToolTip createToolTip() {
				return new MyTipTool();
	...
	private JPanel createComplexSizePanel() {
		...
		complexSizeThreshold.setText((new Integer(currentParameters
				.getComplexSizeThresholdEAGLE()).toString()));
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (source == cliqueSizeThreshold) {
				...
				if ((value != null) && (value.intValue() >= 0)
						&& (value.intValue() <= 10)) {
					currentParameters.setCliqueSizeThresholdEAGLE(value
							.intValue());
				} else {
					// source.setValue(new Double
					// (currentParameters.getCliqueSizeThresholdEAGLE()));
				...
				if ((value != null) && (value.intValue() >= 0)) {
					currentParameters.setComplexSizeThresholdEAGLE(value
							.intValue());
				} else {
					// source.setValue(new Double
					// (currentParameters.getComplexSizeThresholdEAGLE()));
