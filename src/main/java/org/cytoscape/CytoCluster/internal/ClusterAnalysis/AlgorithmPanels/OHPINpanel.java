...
public class OHPINpanel extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7133001655891167288L;
	...
	public OHPINpanel(CySwingApplication swingApplication, ClusterUtil mcodeUtil) {
		...
		collapsiblePanel.getContentPane().add(cliqueSizePanel,
				BorderLayout.NORTH);
		collapsiblePanel.getContentPane().add(OverlappingScorePanel,
				BorderLayout.CENTER);
		collapsiblePanel
				.setToolTipText("Customize parameters for HC-PIN (Optional)");
	...
	private JPanel createCliqueSizePanel() {
		...
		fThreshold.addPropertyChangeListener("value",
				new OHPINpanel.FormattedTextFieldAction());
		...
		fThreshold.setText((new Double(currentParameters.getfThresholdOHPIN())
				.toString()));
		// the panel
		panel = new JPanel() {
			public JToolTip createToolTip() {
	...
	private JPanel createOverlappingScorePanel() {
		...
		OverlappingScore.addPropertyChangeListener("value",
				new OHPINpanel.FormattedTextFieldAction());
		String tip3 = "evaluate the overlap of two clusters\n"
				+ "usually set to 0.5";
		OverlappingScore.setToolTipText(tip3);
		OverlappingScore.setText((new Double(currentParameters
				.getOverlappingScore()).toString()));
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			if (source == fThreshold) {
				...
				if ((value != null) && (Double.parseDouble(value) >= 0.0)) {
					currentParameters.setOverlappingScore(Double
							.parseDouble(value));
				} else {
