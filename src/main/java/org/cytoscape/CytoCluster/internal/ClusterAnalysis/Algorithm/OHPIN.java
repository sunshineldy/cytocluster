...
public class OHPIN extends Algorithm {
	...
	public ArrayList<Cluster> calC_set(CyNetwork inputNetwork) {
		...
		while (edges.hasNext()) {
			...
			if (B_cluster != null) {
				if (!C_set.isEmpty()) {
					...
					for (int i = 0; i < C_set.size(); i++) {
						...
						if (isBelong(B_cluster, ci)) {// if B_cluster belongs to
														// ci
							B_cluster.setAdd(0);
							break;
						}
			...
			if (taskMonitor != null) {
				findingProgress++;
				// We want to be sure that only progress changes are reported
				// and not
				// miniscule decimal increments so that the taskMonitor isn't
				// overwhelmed
	...
	public double calCommonedgeW(Cluster c1, Cluster c2, CyNetwork inputNetwork) {
		...
		while (nodes.hasNext()) {
			Long n = nodes.next();
			ArrayList<CyEdge> edges = (ArrayList<CyEdge>) inputNetwork
					.getAdjacentEdgeList(inputNetwork.getNode(n), Type.ANY);
			for (int i = 0; i < edges.size(); i++) {
				CyEdge e = edges.get(i);
				if (allnodes2.contains(e.getSource().getSUID())
						|| allnodes2.contains(e.getSource().getSUID()))
					if (inputNetwork.getRow(e).get("weight", Double.class) != null)
						sum += inputNetwork.getRow(e).get("weight",
								Double.class);
					else
						sum += 1.0;
			}
	...
	public double calCCV(Cluster c1, Cluster c2, CyNetwork inputNetwork) {
		double ccv = calCommonedgeW(c1, c2, inputNetwork)
				/ (c1.getALNodes().size() * c2.getALNodes().size());
		return ccv;
	}

	public boolean isModule(Cluster c1, double thereshold,
		...
		while (nodes.hasNext()) {
			...
			for (int i = 0; i < edges.size(); i++) {
				CyEdge e = edges.get(i);
				if (allnodes.contains(e.getSource().getSUID())
						|| allnodes.contains(e.getSource().getSUID()))
					indegree++;
				else
					outdegree++;
			}
	...
	public ArrayList<Cluster> ModuleMerge(ArrayList<Cluster> C_set,
			double thereshold, CyNetwork inputNetwork) {
		int s = C_set.size();
		double[][] ccvmatrix = new double[s][s];

		PriorityQueue<CCV> ccvq = new PriorityQueue<CCV>(11,
				new Comparator<CCV>() {
	...
	public Cluster[] OHPINFinder(CyNetwork inputNetwork, int resultTitle) {
		...
		for (int t = 0; t < C_set.size(); t++) {
			...
			while (ii.hasNext()) {
				...
				System.out.println(x + "    "
						+ inputNetwork.getRow(nn).get("name", String.class));
				x++;
			}
		}
		...
		ArrayList alOriginalClusters = ModuleMerge(C_set,
				params.getfThresholdOHPIN(), inputNetwork);

		/*
		 * ArrayList alClusters1 = calC_set(currentNetwork); //Finally convert
		 * the arraylist into a fixed array Cluster[] clusters = new
		 * Cluster[alClusters1.size()]; for (int c = 0; c < clusters.length;
		 * c++) { clusters[c] = (Cluster) alClusters1.get(c); } long msTimeAfter
		 * = System.currentTimeMillis(); lastFindTime = msTimeAfter -
		 * msTimeBefore;
		 * 
		 * for(int t=0; t<clusters.length; t++){
		 * 
		 * Iterator ii = clusters[t].getALNodes().iterator();
		 * System.out.println("cluster "+t); int x=1; while(ii.hasNext()){
		 * CyNode nn = inputNetwork.getNode((Long)ii.next());
		 * System.out.println(
		 * x+"    "+inputNetwork.getRow(nn).get("name",String.class)); x++; } }
		 * 
		 * 
		 * 
		 * alClusters1 = OSMerge(alClusters1, params.getOverlappingScore());
		 * System.out.println(alClusters1.size()+"@@@@@@"); //Finally convert
		 * the arraylist into a fixed array clusters = new
		 * Cluster[alClusters1.size()]; for (int c = 0; c < clusters.length;
		 * c++) { clusters[c] = (Cluster) alClusters1.get(c); } msTimeAfter =
		 * System.currentTimeMillis(); lastFindTime = msTimeAfter -
		 * msTimeBefore;
		 * 
		 * for(int t=0; t<clusters.length; t++){
		 * 
		 * Iterator ii = clusters[t].getALNodes().iterator();
		 * System.out.println("cluster "+t+"*******"); int x=1;
		 * while(ii.hasNext()){ CyNode nn =
		 * inputNetwork.getNode((Long)ii.next());
		 * System.out.println(x+"    "+inputNetwork
		 * .getRow(nn).get("name",String.class)); x++; } }
		 * 
		 * alClusters1 = ModuleMerge(alClusters1, params.getfThreshold(),
		 * inputNetwork); System.out.println(alClusters1.size()+"@@@@@@");
		 * //Finally convert the arraylist into a fixed array clusters = new
		 * Cluster[alClusters1.size()]; for (int c = 0; c < clusters.length;
		 * c++) { clusters[c] = (Cluster) alClusters1.get(c); } msTimeAfter =
		 * System.currentTimeMillis(); lastFindTime = msTimeAfter -
		 * msTimeBefore;
