...
import org.cytoscape.CytoCluster.internal.MyUtils.NodeDistances;
 ...
 * Note: this was copied from giny.util because it is being phased out.
 * Eventually the layout API will be available to use remove when layout API is
 * available)
 */
public class SpringEmbeddedLayouter {
	...
	protected HashMap<Long, Integer> nodeIndexToMatrixIndexMap; // maps a root
																// graph index
																// to a distance
																// matrix index
	protected TreeMap<Integer, Long> matrixIndexToNodeIndexMap; // maps a
																// distance
																// matrix index
																// to a root
																// graph
	...
	protected void initializeSpringEmbeddedLayouter() {
		...
		/*
		 * nodeCount = graphView.getNodeViewCount(); edgeCount =
	...
	public void calculate() {
		...
		/*
		 * // initialize the layouting. nodeCount =
		 * graphView.getNodeViewCount(); edgeCount =
		 * graphView.getEdgeViewCount();
		 */
		// initialize progress
		...
		Iterator nodes = ((CyNetwork) graphView.getModel()).getNodeList()
				.iterator();
		...
		/*
		 * View node_view; PartialDerivatives partials;
		 */
		...
		try {
			for (layoutPass = 0; layoutPass < numLayoutPasses; layoutPass++) {
				...
				for (Iterator iterator = graphView.getNodeViews().iterator(); iterator
						.hasNext();) {
					View node_view = (View) iterator.next();
					PartialDerivatives partials = new PartialDerivatives(
							node_view);
				...
				/*
				 * node_views_iterator = graphView.getNodeViewsIterator(); while
				 * (node_views_iterator.hasNext()) { node_view = (NodeView)
				 * node_views_iterator.next();
				 * 
				 * partials = new PartialDerivatives(node_view);
				 * calculatePartials(partials, null, potential_energy, false);
				 * partials_list.add(partials); if ((furthest_node_partials ==
				 * null) || (partials.euclideanDistance >
				 * furthest_node_partials.euclideanDistance) ) { //
				 * //System.out.println(
				 * "P: "+furthest_node_partials.euclideanDistance
				 * +" E: "+partials.euclideanDistance ); furthest_node_partials
				 * = partials; } }
				 */

				// Until num_iterations, or the furthest node is not-so-fur,
				// move the
				...
				for (int iterations_i = 0; ((iterations_i < num_iterations) && (furthest_node_partials.euclideanDistance >= euclidean_distance_threshold)); iterations_i++) {
					...
					// Calculate Percentage. This must be a value between
					// 0..100.
					...
					// This automatically updates the UI Component w/ progress
					// bar.
					if (taskMonitor != null) {
						taskMonitor.setProgress(percentComplete);
						taskMonitor
								.setStatusMessage("Calculating network layout: "
										+ currentProgress + "of " + maxValue);
					...
					currentProgress++;
				} // End for each iteration, attempt to minimize the total
					// potential
					// energy by moving the node that is furthest from where it
					// should be.
	...
	public boolean doLayout(double weightLayout, double goalTotal,
			double progress, Loader loader) {
		nodeCount = ((CyNetwork) graphView.getModel()).getNodeCount();
		edgeCount = ((CyNetwork) graphView.getModel()).getEdgeCount();
		nodeIndexToMatrixIndexMap = new HashMap();
		matrixIndexToNodeIndexMap = new TreeMap();
		...
		for (layoutPass = 0; layoutPass < numLayoutPasses; layoutPass++) {
			...
			for (int iterations_i = 0; iterations_i < num_iterations
					&& furthest_node_partials.euclideanDistance >= euclidean_distance_threshold; iterations_i++) {
				if (interrupted) {
				...
				furthest_node_partials = moveNode(furthest_node_partials,
						partials_list, potential_energy);
				progress += 100D
						* (1.0D / (double) (num_iterations * numLayoutPasses))
						* (weightLayout / goalTotal);
				if (loader != null)
	...
	protected void setupNodeDistanceSprings() {
		...
		Collection<Long> matrixIndices = this.matrixIndexToNodeIndexMap
				.values();
		int i = 0;

		for (Long nodeId : matrixIndices) {
			nodeList.add(i, ((CyNetwork) this.graphView.getModel())
					.getNode(nodeId.longValue()));
			i++;
		}

		NodeDistances ind = new NodeDistances(nodeList,
				(CyNetwork) this.graphView.getModel(),
				this.nodeIndexToMatrixIndexMap);
		...
		/*
		 * // We only have to do this once. if (layoutPass != 0) { return; }
		 * 
		 ...
		 * nodeIndexToMatrixIndexMap ArrayList nodeList = new ArrayList();
		 * Collection matrixIndices = matrixIndexToNodeIndexMap.values(); int i
		 * = 0; for (Iterator iterator = matrixIndices.iterator();
		 * iterator.hasNext();) { Integer nodeIndex = (Integer) iterator.next();
		 * nodeList.add(i,
		 * ((CyNetwork)graphView.getModel()).getNode(nodeIndex.longValue()));
		 * nodeList.add(i,
		 * graphView.getGraphPerspective().getNode(nodeIndex.intValue())); i++;
		 * } NodeDistances ind = new NodeDistances(nodeList,
		 * (CyNetwork)graphView.getModel(), nodeIndexToMatrixIndexMap);
		 * NodeDistances ind = new NodeDistances(nodeList,
		 * graphView.getGraphPerspective(), nodeIndexToMatrixIndexMap); int[][]
		 * node_distances = (int[][]) ind.calculate();
		 * 
		 * if (node_distances == null) { return; }
		 * 
		 * // TODO: A good strength_constant is the characteristic path length
		 * of the // graph. For now we'll just use nodeDistanceStrengthConstant.
		 * double node_distance_strength_constant =
		 ...
		 * ); // To make it bigger, try // rest_length_constant =
		 * graphView.averageEdgeLength(); // To make it smaller, try //
		 * rest_length_constant = Math.sqrt( ( graphView.getViewRect().width *
		 * graphView.getViewRect.height() ) / graphView.getGraphDiameter() ); //
		 * For now we'll just use nodeDistanceRestLengthConstant. double
		 * node_distance_rest_length_constant = nodeDistanceRestLengthConstant;
		 * 
		 * // Calculate the rest lengths and strengths based on the node
		 * distance data for (int node_i = 0; node_i < nodeCount; node_i++) {
		 ...
		 * disconnectedNodeDistanceSpringRestLength; //System.out.println(
		 ...
		 * node_distances[node_i][node_j]); //System.out.println(
		 ...
		 * nodeDistanceSpringStrengths[node_i][node_j] =
		 * (node_distance_strength_constant / (node_distances[node_i][node_j] *
		 * node_distances[node_i][node_j]) ); } // Mirror over the diagonal.
	...
	protected PartialDerivatives calculatePartials(PartialDerivatives partials,
		...
		int node_view_index = ((Integer) nodeIndexToMatrixIndexMap
				.get(((CyNode) node_view.getModel()).getSUID())).intValue();
		double node_view_radius = ((Double) node_view
				.getVisualProperty(BasicVisualLexicon.NODE_WIDTH))
				.doubleValue();
		double node_view_x = ((Double) node_view
				.getVisualProperty(BasicVisualLexicon.NODE_X_LOCATION))
				.doubleValue();
		double node_view_y = ((Double) node_view
				.getVisualProperty(BasicVisualLexicon.NODE_Y_LOCATION))
				.doubleValue();

		/*
		 * int node_view_index = ((Integer) nodeIndexToMatrixIndexMap.get( new
		 * Integer(node_view.getRootGraphIndex()))).intValue(); double
		 * node_view_radius = node_view.getWidth(); double node_view_x =
		 * node_view.getXPosition(); double node_view_y =
		...
		while (iterator.hasNext()) {
			...
			if (((CyNode) node_view.getModel()).getSUID() == ((CyNode) other_node_view
					.getModel()).getSUID()) {
				// if (node_view.getRootGraphIndex() ==
				// other_node_view.getRootGraphIndex()) {
				// System.out.println( "Nodes are the same. " );
				continue;
			}

			other_node_view_index = ((Integer) nodeIndexToMatrixIndexMap
					.get(((CyNode) other_node_view.getModel()).getSUID()))
					.intValue();
			other_node_view_radius = Math.max(((Double) other_node_view
					.getVisualProperty(BasicVisualLexicon.NODE_WIDTH))
					.doubleValue(), ((Double) other_node_view
					.getVisualProperty(BasicVisualLexicon.NODE_HEIGHT))
					.doubleValue());
			delta_x = node_view_x
					- ((Double) other_node_view
							.getVisualProperty(BasicVisualLexicon.NODE_X_LOCATION))
							.doubleValue();
			delta_y = node_view_y
					- ((Double) other_node_view
			 ...
			 * 
			 * delta_x = (node_view_x - other_node_view.getXPosition()); delta_y
			 * = (node_view_y - other_node_view.getYPosition());
			 */
	...
	protected void simpleMoveNode(PartialDerivatives partials) {
		/*
		 * View node_view = partials.getNodeView(); double denomenator =
		 * ((partials.xx * partials.yy) - (partials.xy * partials.xy)); double
		 * delta_x = ( ((-partials.x * partials.yy) - (-partials.y *
		 * partials.xy)) / denomenator ); double delta_y = ( ((-partials.y *
		 * partials.xx) - (-partials.x * partials.xy)) / denomenator );
		 * 
		 * // REMOVE //System.out.println( "moving node \"" + node_view +
		 * "\" to ( " + ( node_view.getXPosition() + delta_x ) + ", " + (
		 * node_view.getYPosition() + delta_y ) + " )." );
		 * 
		 * // TODO: figure out movement //node_view.setXPosition( //
		 * node_view.getXPosition() + delta_x //); //node_view.setYPosition( //
		 * node_view.getYPosition() + delta_y //);
		 * 
		 * Point2D p = node_view.getOffset(); node_view.setOffset(p.getX() +
		 * delta_x, p.getY() + delta_y);
		 */

		View node_view = partials.getNodeView();
		double denomenator = partials.xx * partials.yy - partials.xy
				* partials.xy;
		double delta_x = (-partials.x * partials.yy - -partials.y * partials.xy)
				/ denomenator;
		double delta_y = (-partials.y * partials.xx - -partials.x * partials.xy)
				/ denomenator;
		double x = ((Double) node_view
				.getVisualProperty(BasicVisualLexicon.NODE_X_LOCATION))
				.doubleValue();
		double y = ((Double) node_view
				.getVisualProperty(BasicVisualLexicon.NODE_Y_LOCATION))
				.doubleValue();
		node_view.setVisualProperty(BasicVisualLexicon.NODE_X_LOCATION,
				Double.valueOf(x + delta_x));
		node_view.setVisualProperty(BasicVisualLexicon.NODE_Y_LOCATION,
				Double.valueOf(y + delta_y));
