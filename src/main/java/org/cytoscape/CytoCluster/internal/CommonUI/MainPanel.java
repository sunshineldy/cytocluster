...
import org.cytoscape.CytoCluster.internal.ClusterAnalysis.AlgorithmPanels.*;
...
public class MainPanel extends JPanel implements CytoPanelComponent {
	...
	public MainPanel(CySwingApplication swingApplication, ClusterUtil mcodeUtil) {
		...
		this.currentParamsCopy = this.mcodeUtil.getCurrentParameters()
				.getParamsCopy(null); // netWorkId initial value is null
		...
		this.add(createScopePanel(), BorderLayout.NORTH);
	...
	private JPanel createScopePanel() {
		if (this.scopePanel == null) {
			...
			this.scopePanel.setLayout(new BoxLayout(this.scopePanel,
					BoxLayout.Y_AXIS));
			this.scopePanel.setBorder(BorderFactory
					.createTitledBorder("Find Cluster(s)"));

			JRadioButton scopeNetwork = new JRadioButton("in Whole Network",
					this.currentParamsCopy.getScope().equals(
							ParameterSet.NETWORK));
			JRadioButton scopeSelection = new JRadioButton("from Selection",
					this.currentParamsCopy.getScope().equals(
							ParameterSet.SELECTION));
	...
	private JScrollPane createAlgorithmPanel() {
		...
		JPanel choicePanelNonOverlapping = new JPanel();
		JPanel choicePanelOverlapping = new JPanel();
		choicePanelNonOverlapping.setLayout(new BoxLayout(
				choicePanelNonOverlapping, BoxLayout.Y_AXIS));
		choicePanelOverlapping.setLayout(new BoxLayout(choicePanelOverlapping,
				BoxLayout.Y_AXIS));

		choicePanelNonOverlapping.setPreferredSize(new Dimension(260, 100));
		choicePanelOverlapping.setPreferredSize(new Dimension(260, 100));

		choicePanelNonOverlapping.setBorder(BorderFactory
				.createTitledBorder("NonOverlapping"));
		choicePanelOverlapping.setBorder(BorderFactory
				.createTitledBorder("Overlapping"));

		JRadioButton MCODEButton = new JRadioButton("MCODE", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.MCODE));
		JRadioButton EAGLEButton = new JRadioButton("EAGLE", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.EAGLE));
		JRadioButton FAGECButton = new JRadioButton("FAG-EC", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.FAGEC));
		JRadioButton HCPINButton = new JRadioButton("HC-PIN", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.HCPIN));
		JRadioButton OHPINButton = new JRadioButton("OH-PIN", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.HCPIN));
		JRadioButton IPCAButton = new JRadioButton("IPCA", currentParamsCopy
				.getAlgorithm().equals(ParameterSet.IPCA));

		MCODEButton
				.setToolTipText("Use K-Core-based MCODE algorithm.\nA K-Core is a subgraph with minimum degree of k");
		EAGLEButton
				.setToolTipText("Use maximal clique-based EAGLE algorithm.\n Overlapped clusters can be identified");
		FAGECButton
				.setToolTipText("Use fast hierarchical agglomerative FAG-EC algorithm");
		HCPINButton
				.setToolTipText("Use A fast hierarchical clustering algorithm .\n for functional modules discovery in protein .\n interaction networks");
		OHPINButton
				.setToolTipText("Use A fast hierarchical clustering algorithm .\n for functional modules discovery in protein .\n interaction networks");
		IPCAButton
				.setToolTipText("Use A clustering algorithm based on the new topological structure makes it possible to \n for identifing dense subgraphs in protein interaction networks .\n ");
		...
		choicePanelNonOverlapping.add(MCODEButton);
		choicePanelNonOverlapping.add(FAGECButton);
		choicePanelNonOverlapping.add(HCPINButton);
		choicePanelOverlapping.add(OHPINButton);
		choicePanelOverlapping.add(IPCAButton);
		choicePanelOverlapping.add(EAGLEButton);

		choicePanel.setToolTipText("Please select an algorithm");
		choicePanel.add(choicePanelNonOverlapping, BorderLayout.NORTH);
		choicePanel.add(choicePanelOverlapping, BorderLayout.CENTER);

		JPanel options = new JPanel();
		options.setLayout(new BoxLayout(options, BoxLayout.Y_AXIS));
		MCODE = createOptionsPanel1();
		MCODE.setVisible(currentParamsCopy.getAlgorithm().equals("MCODE"));

		// JOptionPane.showMessageDialog(null,
		// "" + currentParamsCopy.getAlgorithm());
	 ...
	 * inputing scoring and clustering parameter
	 * 
	...
	private CollapsiblePanel createScoringPanel() {
		...
		includeLoops
				.addItemListener(new MainPanel.IncludeLoopsCheckBoxAction());
	...
	private CollapsiblePanel createClusterParaPanel(JRadioButton component) {
		...
		nodeScoreThreshold.setText((new Double(currentParamsCopy
		...
		kCore.setColumns(3);
		kCore.addPropertyChangeListener("value",
				new MainPanel.FormattedTextFieldAction());
		...
		nodeDensityThreshold.setText((new Double(currentParamsCopy
		...
		maxDepth.setText((new Integer(currentParamsCopy.getMaxDepthFromStart())
				.toString()));
	...
	private JPanel createBottomPanel() {
		...
		/*
		 * analyzeButton.addActionListener(new AnalyzeAction( title,
		 * cyApplicationManagerServiceRef, desktopApp, CyNetworkViewManager
		 * netViewManager, CyServiceRegistrar registrar, TaskManager
		 * taskManager, currentParamsCopy, ClusterUtil util));
		 */
	...
	/**
	 * Handles the press of a algorithm option. Makes sure that appropriate
	 * options inputs are added and removed depending on which algorithm is
	 * selected
	 */
	private class AlgorithmAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			MCODE.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.MCODE));
			EAGLE.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.EAGLE));
			FAGEC.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.FAGEC));
			HCPIN.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.HCPIN));
			OHPIN.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.OHPIN));
			IPCA.setVisible(currentParamsCopy.getAlgorithm().equals(
					ParameterSet.IPCA));
		}
	}
		...
		public void propertyChange(PropertyChangeEvent e) {
			...
			try {
				...
				if (source == degreeThreshold) {
					Number value = format.parse((String) degreeThreshold
							.getValue());
					...
					Number value = format.parse((String) nodeScoreThreshold
							.getValue());
					if ((value != null) && (value.doubleValue() >= 0.0)
							&& (value.doubleValue() <= 1.0)) {
						currentParamsCopy.setNodeScoreCutoff(value
								.doubleValue());
					} else {
						source.setValue(new Double(currentParamsCopy
								.getNodeScoreCutoff()));
					...
					if ((value != null) && (value.intValue() > 0)) {
						currentParamsCopy
								.setMaxDepthFromStart(value.intValue());
					} else {
					...
					Number value = format.parse((String) nodeDensityThreshold
							.getValue());
					if ((value != null) && (value.doubleValue() >= 0.0)
							&& (value.doubleValue() <= 1.0)) {
						currentParamsCopy.setFluffNodeDensityCutoff(value
								.doubleValue());
					} else {
						source.setValue(new Double(currentParamsCopy
				...
				if (invalid) {
					JOptionPane.showMessageDialog(
							MainPanel.this.desktopApp.getJFrame(), message,
							"Parameter out of bounds", 2);
				}
	...
	/*
	 * public ClusterPlugin.MainPanelAction getTrigger() { return trigger; }
	 * 
